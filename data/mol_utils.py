...
def extract_scaffold(x, adj, scaffold_indices):
    scaffold_x = torch.zeros_like(x)

    scaffold_x[scaffold_indices] = x[scaffold_indices]

    scaffold_adj = torch.zeros_like(adj)
    scaffold_indices = torch.tensor(scaffold_indices, dtype=torch.long)
    scaffold_adj[scaffold_indices[:, None], scaffold_indices] = adj[scaffold_indices][:, scaffold_indices]
    # scaffold_edge_index = torch.nonzero(scaffold_adj, as_tuple=False).T
    return scaffold_x, scaffold_adj