...
def load_mol(filepath):
    ...
    while True:
        ...
        if key in load_data.keys():
            ...
            break
    return list(map(lambda x, a, scaffold_indices, y: (x, a, scaffold_indices, y), result[0], result[1], result[2], result[3]))
...
def get_transform_fn(dataset):
    if dataset == 'QM9':
        ...
        def transform(data):
            ...
            y = torch.tensor(y[1:]).to(torch.float32)
            return x, adj, scaffold_x, scaffold_adj, y
    elif dataset == 'ChEMBL':
        def transform(data):
            x, adj, scaffold_indices, y = data
            # the last place is for virtual nodes
            # 6: C, 7: N, 8: O, 9: F, 15: P, 16: S, 17: Cl, 35: Br, 53: I
            chembl_atomic_num_list = [6, 7, 8, 9, 15, 16, 17, 35, 53, 0]
            x_ = np.zeros((48, 10), dtype=np.float32)
            for i in range(48):
                ind = chembl_atomic_num_list.index(x[i])
                x_[i, ind] = 1.
            x = torch.tensor(x_).to(torch.float32)
            # single, double, triple, aromic, and no-bond; the last channel is for virtual edges
            adj = np.concatenate([adj[:4], 1 - np.sum(adj[:4], axis=0, keepdims=True)],
                                 axis=0).astype(np.float32)

            x = x[:, :-1]                               # 9, 5 (the last place is for vitual nodes) -> 9, 4 (38, 9)
            adj = torch.tensor(adj.argmax(axis=0))      # 4, 9, 9 (the last place is for vitual edges) -> 9, 9 (38, 38)
            # 0, 1, 2, 3 -> 1, 2, 3, 0; now virtual edges are denoted as 0
            adj = torch.where(adj == 4, 0, adj + 1).to(torch.float32)
            scaffold_x, scaffold_adj = extract_scaffold(x, adj, scaffold_indices)
            y = torch.tensor([y[10], y[22]]).to(torch.float32)
            return x, adj, scaffold_x, scaffold_adj, y
    return transform
...
def dataloader(config, get_graph_list=False):
    ...
    if get_graph_list:
        train_mols_nx = [nx.from_numpy_array(np.array(adj)) for x, adj, sx, sa, y in train_dataset]
        test_mols_nx = [nx.from_numpy_array(np.array(adj)) for x, adj, sx, sa, y in test_dataset]
