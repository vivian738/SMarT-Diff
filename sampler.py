...
import math
import torch
from pynndescent.pynndescent_ import diversify
from rdkit import Chem
from rdkit.Chem.Scaffolds import MurckoScaffold
from torch.cuda import device

from src.vinadock import prepare_prot, calculate_vina_score
from utils.logger import Logger, set_log, start_log, train_log, sample_log, check_log
from utils.loader import load_ckpt, load_data, load_seed, load_device, load_model_from_ckpt, \
                         load_ema_from_ckpt, load_sampling_fn, load_eval_settings, load_classifer_from_ckpt
...
from src.metrics import eval_graph_list, calculate_diversity
...
class Sampler_mol(object):
    ...
    def sample(self):
        ...
        self.log_folder_name, self.log_dir, _ = set_log(self.configt, is_train=False)
        self.log_name = f"{self.config.ckpt}-sample-{self.config.sample.template}"
        logger = Logger(str(os.path.join(self.log_dir, f'{self.log_name}.log')), mode='a')
        ...
        with open(f'data/{self.config.sample.template}.csv') as f:
            lines = f.readlines()
        scaf_ = []
        for line in lines:
        ...
        for r in range(num_sampling_rounds):
            time.time()
            start = r * self.config.sample.batch_size
            end = min((r+1) * self.config.sample.batch_size, len(scaf_))
            scaf_r = scaf_[start:end]
            self.init_flags, sx, sa = init_flags(self.train_graph_list, self.configt, scaf_r, self.config.sample.batch_size)
            self.init_flags = self.init_flags.to(self.device[0])
            sx = sx.to(self.device[0])
            sa = sa.to(self.device[0])
            # y = torch.tensor([0.75, 2.5]).repeat(self.config.sample.batch_size,1) # training set has the property ZINC250k (QED, SA)

            y = torch.tensor([0.8, 0.61]).repeat(self.config.sample.batch_size,1)  #ChEMBL (QED weight, NP likeness score)
            y = y.to(self.device[0])
            x, adj, _ = self.sampling_fn(self.model, self.init_flags, sx, sa, y)
        ...
        scores = get_all_metrics(gen=gen_smiles, k=len(gen_smiles), device=self.device[0], n_jobs=8, test=test_smiles, test_scaffolds=scaf_smiles, train=train_smiles)
        # pocket_center, box_size, protein_pdbqt = prepare_prot(f'data/{self.config.sample.template.lower()}_pdb/{self.config.sample.template.lower()}.pdb',
        #                                                       f'data/{self.config.sample.template.lower()}_pdb/{self.config.sample.template.lower()}.sdf')
        num_success = 0
        filter_mols = [m for m in gen_mols if m is not None]
        for m in filter_mols:
            s = round((10 - SA(m)) / 9, 2)
            q = QED(m)
            # dock_score = calculate_vina_score(pocket_center, box_size, protein_pdbqt, m)
            # if s > 0.6 and q > 0.4 and dock_score < -8:
            if s > 0.6 and q > 0.67:
                num_success +=1
        ...
        diversity = calculate_diversity(filter_mols)
        ...
        logger.log(f'Diversity: {diversity}')
        logger.log('='*100)
