...
class Attention(torch.nn.Module):
    ...
    def forward(self, x, adj, flags, attention_mask=None): #conditions=None,
...
class AttentionLayer(torch.nn.Module):
    ...
    def forward(self, x, adj, flags): #, conditions=None
        """
        ...
        for _ in range(len(self.attn)):
            if _ == 0:
                _x, mask = self.attn[_](x, adj[:,_,:,:], flags) #, conditions
            else:
